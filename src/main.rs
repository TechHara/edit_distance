...
fn get_runner(atom: Atom, metric: Metric) -> impl Fn(&str, &str) -> f64 {
    match metric {
        ...
        Metric::DL => match atom {
            Atom::Byte => |x: &str, y: &str| {
                let x = x.as_bytes();
                let y = y.as_bytes();
                damerau_levenshtein_distance(x, y) as f64 / x.len().max(y.len()) as f64
            },
            Atom::Char => |x: &str, y: &str| {
                let x: Vec<char> = x.chars().collect();
                let y: Vec<char> = y.chars().collect();
...
fn damerau_levenshtein_distance<T>(x: &[T], y: &[T]) -> usize 
where T: std::cmp::Eq {
    let nx = x.len();
    let ny = y.len();
    let mut memo = vec![usize::MAX; nx*ny];
    damerau_levenshtein_distance_helper(x, y, &mut memo, ny)
}

fn damerau_levenshtein_distance_helper<T>(x: &[T], y: &[T], memo: &mut Vec<usize>, ny: usize) -> usize
where T: std::cmp::Eq {
    if x.len() == 0 || y.len() == 0 {
        return x.len().max(y.len());
    } 
    ...
    memo[idx] = match memo[idx] == usize::MAX {
        ...
        _ => match x.last().unwrap() == y.last().unwrap() {
            true => damerau_levenshtein_distance_helper(&x[..x.len()-1], &y[..y.len()-1], memo, ny),
            false => {
                let insert = damerau_levenshtein_distance_helper(&x[..x.len() - 1], y, memo, ny);
                let delete = damerau_levenshtein_distance_helper(x, &y[..y.len() - 1], memo, ny);
                let replace = damerau_levenshtein_distance_helper(&x[..x.len() - 1], &y[..y.len() - 1], memo, ny);
                let transpose = match x.len() >= 2 && y.len() >= 2 && x[x.len()-1] == y[y.len()-2] && x[x.len()-2] == y[y.len()-1] {
                    true => damerau_levenshtein_distance_helper(&x[..x.len()-2], &y[..y.len()-2], memo, ny),
                    false => insert,
                };
